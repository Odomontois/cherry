...
trait NameResolutionLibrary(val name: String) extends Library with NormValue with RecordValueBase:
  ...
  def members: Vector[(String, NormValue)]
  val map               = LayeredMap.fromVector(members.map((k, v) => RecordKey.Symbol(k) -> v))
  private val memberMap = members.toMap
  ...
  private def getKey(key: RecordKey): Process[NormValue] = key match
    case RecordKey.Symbol(name) => Act.option(memberMap.get(name), Cause.MissingKey(key))
